...
gulp.task("webpack-dev-server", function (callback) {
	...
	myConfig.devtool = "source-map"; // Dev purpose we will use source-map and release purpose we will use eval
	myConfig.debug = true;
...
gulp.task("package", ["clean", "version"], function (done) {
	webpack(webpackConfig, function (err, stats) {
		...
		if (stats.compilation.warnings.length) {
			gutil.log('[WARNING:8000]', stats.compilation.warnings.toString())
		}
		done();
	});
});
